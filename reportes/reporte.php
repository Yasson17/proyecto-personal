...
function sumasdiasemana($fecha,$dias)
{
    ...
    $fechafinal = date('d-m-Y', $total);
    $fechadia = date('l-m-Y', $total);

    return [$fechafinal, $fechadia];
}
...
$Culmina = sumasdiasemana($Fecha_IV,$Calculo-1)[0];
//FECHA DE REINTEGRO

if(isset($CalculoR))
{
$R = sumasdiasemana($Fecha_IV,$CalculoR)[1];
$DesR=explode('-', $R);
$DR=$DesR[0];
...
//FECHA DE REINTEGRO EN CASO DE SER DIA VIERNES
    if ($DR=='Friday') 
    {
        $Reingreso = sumasdiasemana($Fecha_IV,$CalculoR+1)[0];
    }
    else
    {
        $Reingreso = sumasdiasemana($Fecha_IV,$CalculoR)[0];
    }
}
else
{
$R = sumasdiasemana($Fecha_IV,$Calculo)[1];
$DesR=explode('-', $R);
$DR=$DesR[0];
...
//FECHA DE REINTEGRO EN CASO DE SER DIA VIERNES
    if ($DR=='Friday') 
    {
        $Reingreso = sumasdiasemana($Fecha_IV,$Calculo+1)[0];
    }
    else
    {
        $Reingreso = sumasdiasemana($Fecha_IV,$Calculo)[0];
    }
}
